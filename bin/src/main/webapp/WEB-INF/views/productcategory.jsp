								...
								<div class="col-sm-6 col-md-3 col-lg-3">
									<div class="shop-item">
										...
										<c:if test="${ item.product_discount > 0 }">
											<del><fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${item.product_price}"  /> VNĐ</del>   
											 ${ item.product_price - item.product_discount} VNĐ
										</c:if>
										<c:if test="${ item.product_discount == 0 }">
											<fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${item.product_price}"  /> VNĐ
										</c:if>
									</div>
								</div>
							</c:forEach>
